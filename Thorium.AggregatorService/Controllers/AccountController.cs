...
using Thorium.Aggregator.Models.AccountViewModels;
...
namespace Thorium.Aggregator.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpGet]
        [AllowAnonymous]
        public async Task<IActionResult> Login(string returnUrl = null, bool bypass = false)
        {
            // Comes from SPA for registration


            var context = await _interaction.GetAuthorizationContextAsync(returnUrl);
            
            // Auto Route to Register 
            if (context?.ClientId == "client_web" && !bypass)
            {
...
//            vm.ReturnUrl = qs.Get("redirect_uri");
//            
//            var returnUri = new UriBuilder(vm.ReturnUrl);
//
//            returnUri.Query = string.Join('&', qs.AllKeys.Select(k => $"{k}={qs[k]}").Skip(1).ToList());
//
//            vm.ReturnUrl = returnUri.ToString();
...
//                    Response.Headers.Remove("Set-Cookie");
        ...
        public async Task<IActionResult> Logout(LogoutViewModel model)
        {
            ...
            await _persistedGrantService.RemoveAllGrantsAsync(subjectId, "back_office_web");
        ...
        public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
        {
            ...
            var isInternal = interaction.ClientId == "back_office_web";
            if (ModelState.IsValid)
            {
                ...
                var anyInternalUsers = _userManager.Users
                    .Any(c => c.IsInternal);
                var result = await _userManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
                    
                    if (!anyInternalUsers && isInternal)
                    {
                        _authorizationService.GetOrCreateSuperUser(user.Id);
                    }

                    if (!isInternal)
                    {
                        _authorizationService.AssignWebsiteUser(user.Id);
                        
                        // Do auto sign-in after registration.
        ...
        //
        // GET: /Account/Register
        [HttpGet("Sys/Account/Register")]
        [AllowAnonymous]
        public IActionResult SysAcountRegister(string returnUrl = null)
        {
            ViewData["ReturnUrl"] = returnUrl;
            return View(nameof(SysAcountRegister));
        }
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                _logger.LogInformation("USending Password Reset Request");
                ...
                await _emailSender.SendEmail(
                   ...
                   "Hi");
                _logger.LogInformation(" Password Reset Request Sent");
                return View("ForgotPasswordConfirmation");
            }
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            _logger.LogInformation("Reset code " + model.Code);
